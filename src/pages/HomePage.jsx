...
const focusDark = "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#ff6b6e] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0b0b0c]";
const focusLight = "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#b9252d] focus-visible:ring-offset-2 focus-visible:ring-offset-[#f7f4f0]";
      ...
      <section className="relative isolate overflow-hidden bg-[#0b0b0c] text-white">
        ...
        <div className="mx-auto grid min-h-[760px] max-w-[1680px] items-center gap-12 px-5 py-16 sm:px-8 lg:grid-cols-[.96fr_1.04fr] lg:px-12 lg:py-20 xl:px-16">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65 }} className="relative z-10">
            ...
            <div className="mt-9 flex flex-wrap gap-3"><Link to="/companies" className={`inline-flex items-center gap-3 rounded-md bg-[#ff4d50] px-6 py-3.5 text-sm font-bold uppercase tracking-[0.06em] text-white transition duration-300 hover:-translate-y-0.5 hover:bg-[#e83c40] ${focusDark}`}>Explore our businesses <span>→</span></Link><Link to="/about" className={`inline-flex items-center rounded-md border border-white/20 bg-white/[0.04] px-6 py-3.5 text-sm font-bold uppercase tracking-[0.06em] text-white transition duration-300 hover:-translate-y-0.5 hover:border-white/35 hover:bg-white/10 ${focusDark}`}>About RB Corp</Link></div>
          </motion.div>

          <motion.div initial={{ opacity: 0, x: 28 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.75, delay: 0.08 }} className="relative lg:pl-4">
            <div className="grid min-h-[500px] grid-cols-2 grid-rows-[1.35fr_.85fr] gap-2 overflow-hidden border border-white/10 bg-[#151517] p-2 shadow-2xl shadow-black/35 sm:min-h-[620px] sm:grid-cols-6">
              <Link to={companies[0].href} className={`group relative col-span-1 overflow-hidden sm:col-span-4 ${focusDark}`}><img src={companies[0].artwork} alt="Beyonist, an RB Corp business" loading="eager" fetchPriority="high" decoding="async" className="h-full w-full object-cover opacity-78 transition duration-700 group-hover:scale-[1.04] group-hover:opacity-95" /><div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-black/10" /><span className="absolute bottom-4 left-4 text-[10px] font-bold uppercase tracking-[0.18em] text-white/80">Beauty & skincare</span></Link>
              <Link to={companies[1].href} className={`group relative col-span-1 overflow-hidden sm:col-span-2 ${focusDark}`}><img src={companies[1].artwork} alt="RB Service Connect, an RB Corp business" loading="eager" decoding="async" className="h-full w-full object-cover opacity-74 transition duration-700 group-hover:scale-[1.04] group-hover:opacity-95" /><div className="absolute inset-0 bg-gradient-to-t from-black/65 via-transparent to-black/10" /><span className="absolute bottom-4 left-4 text-[10px] font-bold uppercase tracking-[0.18em] text-white/80">Services</span></Link>
              {companies.slice(2).map((company) => <Link key={company.name} to={company.href} className={`group relative col-span-1 overflow-hidden sm:col-span-2 ${focusDark}`}><img src={company.artwork} alt={`${company.name}, an RB Corp business`} loading="lazy" decoding="async" className="h-full w-full object-cover opacity-72 transition duration-700 group-hover:scale-[1.05] group-hover:opacity-95" /><div className="absolute inset-0 bg-gradient-to-t from-black/72 via-transparent to-transparent" /><span className="absolute bottom-3 left-3 text-[9px] font-bold uppercase tracking-[0.16em] text-white/80">{company.sector}</span></Link>)}
            </div>
            ...
            <div className="relative -mt-20 ml-4 border border-white/12 bg-[#0b0b0c]/95 p-5 shadow-2xl backdrop-blur-md sm:ml-10 sm:max-w-[560px] sm:p-6 lg:ml-[-3rem]">
              <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
                ...
                <Link to="/about" className={`text-[10px] font-bold uppercase tracking-[0.12em] text-[#ff6b6e] transition hover:text-white ${focusDark}`}>Discover RB Corp →</Link>
              </div>
