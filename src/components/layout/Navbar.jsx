    ...
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur-xl">
      <div className="mx-auto flex h-[84px] max-w-[1680px] items-center justify-between px-5 sm:px-8 lg:px-12 xl:px-16">
        ...
        <nav className="hidden items-center gap-7 xl:flex" aria-label="Primary navigation">
          {links.map(([label, to]) => (
            <NavLink key={to} to={to} className={({ isActive }) => `relative py-8 text-[12px] font-bold uppercase tracking-[0.08em] transition ${isActive ? "text-[#b4842f] after:absolute after:inset-x-0 after:bottom-0 after:h-0.5 after:bg-[#b4842f]" : "text-slate-700 hover:text-[#071a35]"}`}>
              {label}
            </NavLink>
          ))}
        ...
        <button type="button" className="rounded-md border border-slate-300 px-4 py-2 text-xs font-bold uppercase tracking-[0.08em] text-[#071a35] xl:hidden" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-controls="mobile-navigation">
          {open ? "Close" : "Menu"}
        </button>
      </div>
      ...
      <AnimatePresence initial={false}>
        {open && (
          <motion.nav id="mobile-navigation" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden border-t border-slate-200 bg-white xl:hidden">
            <div className="mx-auto flex max-w-[1680px] flex-col px-5 py-3 sm:px-8">
              {[...links, ["Contact", "/contact"]].map(([label, to]) => (
                <Link key={to} to={to} onClick={() => setOpen(false)} className="flex items-center justify-between border-b border-slate-100 py-4 text-sm font-semibold text-slate-700 last:border-none">
